...
class TestPPOCRv3Test(object):
    def setup_class(self):
        ...
        self.util = FastdeployTest(data_dir_name="ICDAR2017", 
            ...
            model_name="PPOCRv3",
            ground_truth="./infer_result/PPOCRv3_ICDAR2017_10_results.txt",
        ...
        # Cls Model
        self.cls_model_path = os.path.join(self.util.model_path, "ch_PP-ch_ppocr_mobile_v2.0_cls_infer") 
        ...
        self.ground_truth = self.util.ground_truth
    ...
    def test_paddle_gpu(self):
        ...
        ppocr_diff_check(model, self.image_file_path, self.ground_truth , model_name=self.model_name, case_name="test_paddle_gpu", csv_path=self.csv_save_path)
    ...
    def test_openvino_cpu(self):
        ...
        rec_model = fd.vision.ocr.Recognizer(
            ...
            runtime_option=self.option)
        model = fd.vision.ocr.PPOCRv3(det_model=det_model, cls_model=cls_model, rec_model=rec_model)
        ppocr_diff_check(model, self.image_file_path, self.ground_truth , model_name=self.model_name, case_name="test_openvino_cpu", csv_path=self.csv_save_path)
    ...
    def test_ort_gpu(self):
        ...
        rec_model = fd.vision.ocr.Recognizer(
            ...
            runtime_option=self.option)
        model = fd.vision.ocr.PPOCRv3(det_model=det_model, cls_model=cls_model, rec_model=rec_model)
        ppocr_diff_check(model, self.image_file_path, self.ground_truth , model_name=self.model_name, case_name="test_ort_gpu", csv_path=self.csv_save_path)
